#!/usr/bin/env python3
"""Build per-player WNBA stats from ESPN actuals backtest data."""
import json, os, statistics
from collections import defaultdict

backtest_dir = '/home/workspace/Daily_Log/backtests/'
stats = defaultdict(lambda: {
    'pts': [], 'reb': [], 'ast': [], 'stl': [], 'blk': [], 'm3': [], 'min': [],
    'team': '', 'games': 0, 'active_games': 0
})

for bt in sorted(os.listdir(backtest_dir)):
    path = os.path.join(backtest_dir, bt, 'espn_actuals_wnba.json')
    ...
    for entry in data:
        ...
        s['m3'].append(entry.get('3PM', 0) or 0)
...
def safe_avg(values, recent=5):
    """Average of last N non-zero entries, fall back to all."""
...
for name, s in stats.items():
    if s['active_games'] < 3:
        continue  # skip players with almost no real minutes
    output[name] = {
        'team': s['team'],
        'games': s['games'],
        ...
        'season': {
            ...
            '3PM': round(statistics.mean(s['m3']), 1),
        },
        'recent5': {
            'PTS': safe_avg(s['pts']),
            'REB': safe_avg(s['reb']),
...
out_path = '/home/workspace/data/wnba_player_stats.json'
os.makedirs(os.path.dirname(out_path), exist_ok=True)
with open(out_path, 'w') as f:
    json.dump(output, f, indent=2)

print(f"Built stats for {len(output)} WNBA players (3+ active games)")
print(f"Saved to {out_path}")

# Show sample
for name in ['P. Bueckers', 'A. Wilson', 'C. Clark', 'A. Stevens', 'S. Dolson', 'J. Jocyte']:
    if name in output:
        p = output[name]
        print(f"  {name:25s} {p['team']:4s}  {p['active_games']}/{p['games']}g  MIN={p['avg_min']:5.1f}  PTS={p['season']['PTS']:5.1f}  PRA={p['season']['PRA']:5.1f}  R5PTS={p['recent5']['PTS']:5.1f}")
