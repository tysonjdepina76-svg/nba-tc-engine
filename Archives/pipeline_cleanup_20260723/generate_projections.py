...
from src.roster_loader import get_mlb_player_names
...
def generate_mlb(today: str):
    ...
    for game in games:
        ...
        if not away_players:
            away_names = get_mlb_player_names(away)
            home_names = get_mlb_player_names(home)
            if not away_names and not home_names:
                away_names = ["Shohei Ohtani", "Mike Trout", "Aaron Judge", "Juan Soto"]
                home_names = ["Mookie Betts", "Freddie Freeman", "Ronald Acuna", "Jose Ramirez"]
            import random
            r = random.Random(f"{matchup}mlb")
            for name in r.sample(away_names, min(4, len(away_names))):
                if name not in seen:
                    seen.add(name)
                    proj = build_mlb_player_proj(name, away, "BAT")
                    away_players.append(proj)
            for name in r.sample(home_names, min(4, len(home_names))):
                if name not in seen:
                    seen.add(name)
                    proj = build_mlb_player_proj(name, home, "BAT")
                    home_players.append(proj)
