// ARCHIVED: /speaking route from zo.space — removed 2026-07-27
// Original route: https://true.zo.space/speaking (public)
    ...
    <main className="min-h-screen bg-zinc-950 text-white">
        ...
        <section className="max-w-2xl mx-auto">
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-xl p-8 space-y-6">
              ...
              <div>
                <label className="block text-sm text-zinc-400 mb-2">Organization (optional)</label>
                <input
                  type="text"
                  value={form.org}
              ...
              <div>
                <label className="block text-sm text-zinc-400 mb-2">What are you looking for?</label>
                <textarea
                  required
                  value={form.message}
                  onChange={e => setForm({ ...form, message: e.target.value })}
                  rows={4}
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-amber-500 transition-colors resize-none"
                  placeholder="Event type, date, audience, what you're hoping for..."
                />
              </div>
              <button
                type="submit"
                className="w-full bg-amber-600 hover:bg-amber-500 text-black font-bold py-4 rounded-lg transition-colors flex items-center justify-center gap-2"
              >
                <Mail className="w-5 h-5" />
                Send Inquiry
              </button>
              <p className="text-zinc-600 text-xs text-center">
                Opens your email client. Prefer direct? Email tysonjdepina76@gmail.com
              </p>
            </form>
          )}
        </section>
        ...
        <div className="max-w-2xl mx-auto mt-12 grid md:grid-cols-2 gap-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-zinc-400 text-sm">Based in New Bedford, Massachusetts. Willing to travel nationally.</p>
            </div>
          </div>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-zinc-400 text-sm">Keynotes, panels, workshops. Virtual and in-person. Flexible scheduling.</p>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-zinc-800 mt-16">
        <div className="max-w-4xl mx-auto px-6 py-8 text-center">
          <p className="text-zinc-500 text-sm">© 2026 Tyson DePina · New Bedford, MA</p>
        </div>
      </div>
    </main>
  );
