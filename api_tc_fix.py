...
// ── 1. ESPN DraftKings embedded — primary (no API key) ────────────────────────
// ESPN embeds real DraftKings odds at:
//   events[].competitions[].odds[ provider.name=="DraftKings" ]
//   ↳ .moneyline{ home/away { close: { odds } } }
//   ↳ .pointSpread{ home/away { close: { line, odds } } }
//   ↳ .total{ over/under { close: { line } } }
  ...
  try {
    ...
    for (const ev of data.events || []) {
      ...
      for (const o of comp.odds || []) {
        ...
        // Total — line is "o218.5" or "u218.5"
        const totalBlock = o.total || {};
        for (const side of ["over", "under"] as const) {
          const close = (totalBlock as any)[side]?.close;
          if (close?.line) {
...
async function fetchDKOdds(sport: string, away: string, home: string): Promise<{
  ...
  try {
    ...
    for (const ev of events) {
      ...
      let total: number|null = null;
      for (const o of totals) { if (!total) total = o.point; }

      return { total, away_spread: awaySpread, home_spread: homeSpread, away_ml: awayMl, home_ml: homeMl, spread_pick };
    }
...
// ── 3. Merge: ESPN DraftKings primary → Odds API fallback ────────────────────────
async function getBestOdds(sport: string, away: string, home: string) {
  ...
  if (espn.away_ml != null || espn.home_ml != null || espn.total != null) {
    return {
      total: espn.total,
      spread: espn.away_spread,
      away_spread: espn.away_spread,
      home_spread: espn.home_spread,
      away_ml: espn.away_ml,
      home_ml: espn.home_ml,
      spread_pick: espn.spread_pick,
    };
  }
  if (ODDS_API_KEY) {
    ...
    return {
      total: dk.total ?? null,
      spread: dk.away_spread ?? null,
      away_spread: dk.away_spread ?? null,
      home_spread: dk.home_spread ?? null,
      away_ml: dk.away_ml ?? null,
      home_ml: dk.home_ml ?? null,
      spread_pick: dk.spread_pick ?? null,
    };
  }
  return { total:null, spread:null, away_spread:null, home_spread:null, away_ml:null, home_ml:null, spread_pick:null };
}
...
async function athleteStats(sport: string, id: string) {
  try { return await getJson(`https://sports.core.api.espn.com/v2/sports/basketball/leagues/${sport.toLowerCase()}/athletes/${id}/statistics?lang=en&region=us`); }
  catch { return null; }
}
...
function annotate(p: P) {
  const out={...p};
  for (const [raw,tcKey,lineKey,edgeKey] of [
    ["pts","tc_pts","line_pts","edge_pts"],["reb","tc_reb","line_reb","edge_reb"],
    ["ast","tc_ast","line_ast","edge_ast"],["tpm","tc_3pm","line_3pm","edge_3pm"],
    ["stl","tc_stl","line_stl","edge_stl"],["blk","tc_blk","line_blk","edge_blk"],
  ] as any[]) {
...
function summarize(players: P[]) {
  ...
  return {
    ...
    bench:{players:bench},
    all:{players:all},
    totals:{tc_pts:sum(active,"tc_pts"),tc_reb:sum(active,"tc_reb"),tc_ast:sum(active,"tc_ast"),
...
function buildLive(sport: string, away: string, home: string, market: number|null, odds: P, awayPlayers: P[], homePlayers: P[]) {
  ...
  const prop_backtest=validPropsFrom(at,ht,sport,away,home);
  return {
    ...
    odds: {
      ...
      ml_source:     odds.away_ml != null ? "ESPN DraftKings embedded" : "ESPN (fallback)",
    },
    assessment:{
      tc_total:tcCombined, tc_line:tcLine, edge, signal,
      total_lean:signal, spread_pick:spreadPick,
      summary:gameMarket
        ?`TC combined ${tcCombined} vs DK ${gameMarket} (edge ${edge>0?"+":""}${edge}) → lean ${signal}. Spread: ${spreadPick}. ML: ${odds.away_ml!=null?away+(odds.away_ml>0?"+"+odds.away_ml:odds.away_ml):"?"} / ${odds.home_ml!=null?home+(odds.home_ml>0?"+"+odds.home_ml:odds.home_ml):"?"}.`
    ...
    away:at, home:ht,
    prop_backtest, valid_props:prop_backtest.valid,
    ...
    for (const statBlock of teamBlock.statistics||[]) {
      ...
      for (const row of statBlock.athletes||[]) {
        ...
        players.push(annotate({
          ...
          status, actual:{pts:parseNum(s.PTS),reb:parseNum(s.REB),ast:parseNum(s.AST),
                          tpm:parse3pt(s["3PT"]),stl:parseNum(s.STL),blk:parseNum(s.BLK)},
...
async function buildHistorical(sport: string, event: string) {
  ...
  const pack=(players:P[])=>({
    ...
    all:{players},
    totals:{actual_pts:round(players.reduce((a,p)=>a+(p.actual?.pts||0),0),1),
  ...
  return {
    ...
    roster_counts:{away:awayPlayers.length,home:homePlayers.length,
