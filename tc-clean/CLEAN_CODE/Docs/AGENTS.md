...
- **Today's run:** 3 games, 19 picks, signals: 1 OVER + 2 UNDER (see last_run.json)
...
## Updates
- **2026-06-15 (WNBA-only combos):** Fixed combo generation to only build for WNBA games (was previously NBA+WNBA). Changed `BASKETBALL` filter to explicit `"WNBA"` check in `daily_picks.py` line 366 and `build_pregame_combos.py` standalone runner. Today's run: 3 WNBA games, 19 picks, 8 qualified combo legs (LV@DAL: 5 qualified — Arike PTS UNDER 14.5 edge 4.4, Chelsea Gray 3PM UNDER 2.5 edge 1.7, Jackie Young/Jewell Loyd/Azzi Fudd 3PM UNDER; POR@MIN: 3 qualified — Carleton/Miles/McBride 3PM UNDER; LA@GS: 0 qualified — no DK lines matched). All combo files WNBA-only confirmed. Git pushed to master.
...
- **2026-06-15 (final):** Pipeline ran — 3 WNBA games, 19 picks: LV@DAL (8 picks, UNDER), POR@MIN (4 picks, UNDER), LA@GS (7 picks, no DK lines). 8 combo-qualified legs. Cleaned all `__pycache__`/`.pyc` directories. Purged 3 stale `.bak` files (tc-workspace/archives nba_tc_engine.py.bak, tc_engine_pre_wnba_live_integration.bak; Projects/wnba_props_live_pull.bak). Git committed (c8d96f0). `/nba-tc` route verified live with 0 errors. WNBA-only combo filter confirmed active.
