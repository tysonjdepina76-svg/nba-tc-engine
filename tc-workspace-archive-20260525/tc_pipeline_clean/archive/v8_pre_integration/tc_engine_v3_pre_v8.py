...
CONS_PTS   = 0.85   # player prop points conservative weight
CONS_REB   = 0.80   # player prop rebounds conservative weight
CONS_AST   = 0.75   # player prop assists conservative weight
CONS_3PM   = 0.70   # player prop 3PM conservative weight

GAP_PTS    = -3.0   # baseline adjustment for pts
GAP_REB    = -1.5   # baseline adjustment for reb
GAP_AST    = -1.0   # baseline adjustment for ast
GAP_3PM    = -0.8   # baseline adjustment for 3pm

LINE_FACTOR = 0.88  # legacy display only — NOT used for game totals
Q_FACTOR    = 0.55  # questionable status
OUT_FACTOR  = 0.0   # out = zero

MIN_EDGE    = 1.0   # minimum edge for prop signal

# ── CONSTANTS: GAME TOTAL v8 (Separate from TC Match) ───────────────────────

STAR_MULTIPLIER = 0.90        # All-NBA first-team players get 0.90 vs general 0.85
ALL_NBA_PLAYERS = {           # star multiplier lookup
    # First Team
    ...
    "Darius Garland": 0.87,
    "Domantas Sabonis": 0.87,
    "Alperen Sengun": 0.87,
}

BENCH_DIFF_THRESHOLD = 15.0   # PPG differential threshold for bench bonus
BENCH_DIFF_BONUS     = 4.0    # pts added per game when team bench diff exceeds threshold

HOME_COURT_BONUS     = 2.0    # pts added to home team game total

MARKET_TOTAL_FLOOR   = True   # use market total as calibration signal (read-only, not a bet trigger)
...
# ── SERIES BENCH TRACKING (for playoff series bench differential) ─────────────

SERIES_BENCH_PTS: Dict[str, Dict[str, float]] = {
    # OKC vs SAS Playoffs 2026
...
class Player:
    ...
    def raw_stat(self, stat: str) -> float:
        stat = stat.lower()
        if stat in ("pts", "points"):   return self.pts
        if stat in ("reb", "rebounds"): return self.reb
        if stat in ("ast", "assists"):  return self.ast
        if stat in ("3pm", "tpm"):      return self.tpm
        raise ValueError(f"Unknown stat: {stat}")

    # ── TC MATCH: Player Props Only ───────────────────────────────────────────

    def tc_prop(self, stat: str) -> float:
        """TC support line for individual player props (PTS/REB/AST/3PM). NOT for game totals."""
    ...
    # ── GAME TOTAL v8: Raw Points ────────────────────────────────────────────

    def raw_points_for_total(self) -> float:
        """Raw point projection for game totals. No TC Match. Uses star multiplier."""
        factor = self.status_factor()
        base = self.pts * factor
        # All-NBA star multiplier
        if self.name in ALL_NBA_PLAYERS:
            base *= STAR_MULTIPLIER
        return round(base, 1)
...
class Team:
    ...
    def prop_tc_totals(self) -> Dict[str, float]:
        """Aggregate TC player prop support. NOT a game total."""
        return {
    ...
    def raw_points_total(self) -> float:
        """Sum of raw points for game totals. No TC Match."""
    ...
    def tc_adjusted_team_total(
        ...
        """
        v8 game total calibration for this team.
        Combines: raw points + All-NBA star multiplier + bench differential + home court.
        Returns dict with adjusted total and breakdown of adjustments applied.
        """
        adj_details = []
        total = 0.0

        for p in self.players:
            raw = p.raw_points_for_total()
            total += raw
        ...
        if series_bench_avg is not None and opponent_bench_avg is not None:
            ...
            if diff > BENCH_DIFF_THRESHOLD:
                bonus = BENCH_DIFF_BONUS
                total += bonus
                adj_details.append(f"+{bonus:.1f} bench_diff ({diff:.1f} PPG)")
...
def calc_series_bench_avg(team_abbr: str, series_data: Dict[str, Dict[str, float]]) -> Optional[float]:
    """Calculate average bench points for a team over a series."""
...
def calc_game_total_v8(
    ...
    """
    v8 Game Total Calibration — separate from TC Match.
    Uses: raw points + All-NBA star multiplier + bench differential + home court.
    Returns adjusted team totals, combined total, gap vs market, and lean.
    """
...
def calc_game(
    ...
    """
    Full game projection: TC Match (player props) + v8 Game Total (separate models).
    Both run independently — they answer different questions.
    """
    home_raw = home.raw_points_total()
    away_raw = away.raw_points_total()
    raw_combined = round(home_raw + away_raw, 1)
    total_gap = round(raw_combined - market_total, 1)
    ...
    # TC Match combined (for prop signal)
    ...
    return {
        ...
        "game_total_v8": v8,
        "raw_points": {
            "home": home_raw, "away": away_raw, "combined": raw_combined,
        },
        "market_total": market_total,
        "total_gap_raw_vs_market": total_gap,
        "spread": {
...
def project_game(
    ...
    """Main projection entry point — returns both TC Match and v8 Game Total."""
    ...
    return {
        ...
        "market_total": market_total,
        "total_gap_raw_vs_market": proj["total_gap_raw_vs_market"],
        "spread": proj["spread"],
        "players": {
        ...
        "bets": {
            ...
            "bankroll": bankroll,
            "note": "Two independent models: (1) TC Match for player props, (2) v8 Game Total for game totals. Both can be used simultaneously.",
        },
    }
...
def _kelly(bankroll: float, edge: float, odds: int = -110,
    ...
    p = min(0.72, 0.52 + min(abs(edge), 10) * 0.02)
    q = 1 - p
...
if FASTAPI_AVAILABLE:
    ...
    class GameRequest(BaseModel):
        home: str; away: str; market_total: float; market_spread: float
