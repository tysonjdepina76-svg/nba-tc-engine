...
OUT_MULT = 0.0
LINE_FACTOR = 0.88
MARKET_LINE_MULT = 1.08  # corrected: real props ≈ stat_avg * 1.05-1.12; was 1.18 (inflated)
MIN_EDGE = 3.0
MIN_HIT_RATE = 0.75
KELLY_FRAC = 0.25
VIG_FACTOR = 0.952  # standard -110 vig for payout calculation

# Backtest calibration from SAS @ OKC WCF Game 1 (21/28 = 75%).
# Keep the TC math itself unchanged: every stat uses stat × 0.85, then target × 0.88.
# These gates only control which props qualify as actionable edges.
STAT_MIN_EDGE = {"PTS": 4.0, "REB": 4.0, "AST": 3.5, "3PM": 1.0}
MIN_TARGET_BY_STAT = {"PTS": 10, "REB": 4, "AST": 2, "3PM": 1}
BENCH_BLOCKED_STATS = {"REB", "AST"}
BENCH_EDGE_BONUS = 2.0
...
def _hydrate_three_point_stats_from_core_api(team_id: str, players: List["Player"]) -> None:
    ...
    for p in players:
        ...
        for season_type in (2, 3):
            try:
                url = (
                    f"https://sports.core.api.espn.com/v2/sports/basketball/nba/"
...
class Player:
    ...
    def prop_edge(self, stat: str, market_line: float) -> Dict[str, Any]:
        """Evaluate an actionable player-prop edge.

        Backtest calibration note:
        - TC math stays fixed: stat × 0.85, target = floor(TC × 0.88).
        - SAS @ OKC WCF Game 1 showed REB volatility (57.1%) and bench REB/AST weakness.
        - Therefore qualification gates are stat/role aware. The projection still prints all 4 stats.
        """
        stat = stat.upper()
        ...
        if self.role != "STARTER":
            if stat in BENCH_BLOCKED_STATS:
                reasons.append("bench REB/AST blocked after SAS@OKC backtest")
            else:
                min_edge += BENCH_EDGE_BONUS
                reasons.append(f"bench edge bonus +{BENCH_EDGE_BONUS}")

        min_target = MIN_TARGET_BY_STAT.get(stat, 1)
        if target < min_target:
            reasons.append(f"target {target} below {stat} minimum {min_target}")

        valid = edge >= min_edge and not any(r.startswith("bench REB/AST") for r in reasons) and target >= min_target

        if edge < min_edge:
            reasons.append(f"edge {edge:.1f} below required {min_edge:.1f}")

        return {
            "stat": stat, "raw_avg": raw, "TC": tc, "T": target,
            "market_line": market_line, "edge": edge, "required_edge": min_edge,
            "valid": valid, "side": "UNDER",
            "note": "TC Match player-prop edge only",
            "calibration_reason": "; ".join(reasons) if reasons else "passes calibrated gates",
        }
...
class Game:
    ...
    def _player_props(self, p: Player) -> Dict[str, Any]:
        ...
        for stat in STAT_KEYS:
            default_line = round(p.stat_value(stat) * MARKET_LINE_MULT, 1)
